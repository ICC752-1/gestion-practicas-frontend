        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_380px]">
          ...
          <form onSubmit={handleCreateUser} className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm h-fit">
            ...
            <div className="mt-5 grid gap-3">
              <input name="email" type="email" required value={form.email} onChange={form.email ? handleFormChange : handleFormChange} placeholder="Correo electrónico" className="rounded-xl border border-gray-200 px-4 py-3 text-sm outline-none focus:border-[#d22864]" />
              ...
              <div className="rounded-2xl border border-gray-100 p-4 bg-gray-50/50">
                ...
                <div className="mt-3 grid gap-2 max-h-[200px] overflow-y-auto pr-1">
                  {roles.map((role) => (
                    <label key={role.id} className="flex items-center gap-3 text-sm font-semibold text-gray-700 cursor-pointer select-none">
                      <input
                        type="checkbox"
                        checked={form.role_ids.includes(role.id)}
                        onChange={() => handleRoleToggle(role.id)}
                        className="rounded text-[#d22864] focus:ring-[#d22864]"
                      />
                      {role.name}
                    </label>
                  ))}
