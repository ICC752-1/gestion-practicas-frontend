...
export const DocumentUploadModal = ({
  ...
  const body = success ? (
    ...
    >
      ...
      >
        {embedded ? 'Subir otro documento' : 'Entendido'}
      </button>
    </motion.div>
  ) : (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="space-y-1.5">
        ...
        >
          <input
            type="file"
            id={embedded ? 'file-upload-embedded' : 'file-upload'}
      ...
      <div className="bg-gradient-to-r from-[#fff0f6] to-white px-8 py-5 border-b border-gray-100 flex justify-between items-center flex-shrink-0 rounded-t-[2rem]">
        ...
        {!embedded && (
          <button
            ...
            aria-label="Cerrar carga de documento"
          >
            <X size={24} />
          </button>
        )}
  ...
  if (embedded) {
    return (
      <section className="overflow-hidden rounded-[2rem] border border-gray-100 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
        {content}
      </section>
    );
  }
