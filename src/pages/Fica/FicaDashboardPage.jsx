      ...
      {rows.map((row) => (
        <div key={row.name}>
          <div className="flex items-center justify-between gap-3 text-sm">
            ...
            <span className="font-black text-gray-900">{row.total} · {formatPercent(row.percentage)}</span>
          </div>
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="flex-grow mx-auto w-full max-w-7xl px-4 py-8">
        <section className="rounded-3xl border border-gray-100 bg-white p-8 shadow-sm">
          <p className="text-sm font-black uppercase tracking-wide text-[#d22864]">Reportes institucionales</p>
          <div className="mt-3 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <h1 className="text-3xl font-black text-gray-900">Dashboard FICA y administrativo</h1>
            ...
            <button
              ...
              className="rounded-2xl bg-gray-900 px-5 py-3 text-sm font-black text-white disabled:opacity-50"
            >
        ...
        <form onSubmit={handleSubmit} className="mt-6 rounded-3xl border border-gray-100 bg-white p-5 shadow-sm">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <input name="date_from" type="date" value={filters.date_from} onChange={handleFilterChange} className="rounded-xl border border-gray-200 px-4 py-3" />
            <input name="date_to" type="date" value={filters.date_to} onChange={handleFilterChange} className="rounded-xl border border-gray-200 px-4 py-3" />
            <input name="career" value={filters.career} onChange={handleFilterChange} placeholder="Carrera" className="rounded-xl border border-gray-200 px-4 py-3" />
            <input name="career_code" value={filters.career_code} onChange={handleFilterChange} placeholder="Código carrera" className="rounded-xl border border-gray-200 px-4 py-3" />
            <select name="practice_type" value={filters.practice_type} onChange={handleFilterChange} className="rounded-xl border border-gray-200 px-4 py-3">
            ...
            <select name="period" value={filters.period} onChange={handleFilterChange} className="rounded-xl border border-gray-200 px-4 py-3">
              <option value="">Periodo</option>
              <option value="Semestre">Semestre</option>
              <option value="Verano">Verano</option>
              <option value="Invierno">Invierno</option>
            </select>
            <select name="status" value={filters.status} onChange={handleFilterChange} className="rounded-xl border border-gray-200 px-4 py-3">
            ...
            <input name="city" value={filters.city} onChange={handleFilterChange} placeholder="Ciudad" className="rounded-xl border border-gray-200 px-4 py-3" />
            <input name="organization" value={filters.organization} onChange={handleFilterChange} placeholder="Organización" className="rounded-xl border border-gray-200 px-4 py-3 lg:col-span-2" />
          </div>
          <div className="mt-4 flex flex-wrap gap-3">
            <button type="submit" disabled={loading} className="rounded-xl bg-[#d22864] px-5 py-3 text-sm font-black text-white disabled:opacity-50">
              {loading ? 'Cargando...' : 'Aplicar filtros'}
            </button>
            <button type="button" onClick={handleReset} className="rounded-xl border border-gray-200 px-5 py-3 text-sm font-black text-gray-700">
              Limpiar
            </button>
          </div>
        </form>
        ...
        {report && (
          <>
            ...
            <section className="mt-6 grid gap-6 lg:grid-cols-3">
              <article className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm">
                <h2 className="text-lg font-black text-gray-900">Tiempos</h2>
                <div className="mt-4 space-y-4">
                  {report.time_metrics.map((metric) => (
                    <div key={metric.name} className="rounded-2xl bg-gray-50 p-4">
                      <p className="font-black text-gray-900">{metric.name}</p>
              ...
              <article className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm">
                <h2 className="text-lg font-black text-gray-900">Documentación y DIRAE</h2>
              ...
              <article className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm">
                <h2 className="text-lg font-black text-gray-900">Alertas</h2>
                <div className="mt-4 grid gap-3 text-sm font-semibold text-gray-700">
            ...
            <section className="mt-6 rounded-3xl border border-gray-100 bg-white p-5 shadow-sm">
              <h2 className="text-lg font-black text-gray-900">Organizaciones recurrentes</h2>
                ...
                <table className="w-full min-w-[680px] text-left text-sm">
                  ...
                  <tbody>
                    {report.recurrent_organizations.map((org) => (
                      <tr key={org.normalized_name} className="border-t border-gray-100">
                        <td className="py-3 font-bold text-gray-900">{org.display_name}</td>
                        <td className="py-3">{org.total}</td>
                        <td className="py-3">{org.approved}</td>
                        <td className="py-3">{org.rejected}</td>
                        <td className="py-3">{org.cancelled}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                {report.recurrent_organizations.length === 0 && <p className="py-4 text-sm font-semibold text-gray-500">No hay organizaciones repetidas para los filtros activos.</p>}
              </div>
            </section>
            ...
            {unavailableNotes.length > 0 && (
              <section className="mt-6 rounded-3xl border border-amber-100 bg-amber-50 p-5 text-sm text-amber-800">
                <h2 className="font-black">Datos no disponibles en este corte</h2>
